...
/// untouched.
///
/// TODO: A future improvement would be a regression test proving `fetchProfile`
/// actually calls `cleanStaleBikes`, which requires stubbing the `Client`
/// network layer.
@MainActor
struct MainContentViewModelTests {
    ...
    @Test func test_bikesWithNilOwnerAreUnaffected() throws {
        ...
        try MainContentPage.ViewModel().cleanStaleBikes(
            myBikeIdentifiers: [],
            currentOwner: currentOwner,
            modelContext: context
        )
...
extension MainContentViewModelTests {
    // MARK: - Helpers
