...
final class PreviewData {
    ...
    static func load<Model: Decodable>(filename: String) throws -> Model? {
        ...
        print("Bundles available are \(Bundle.allBundles.map(\.bundlePath))")
        let scan = Bundle.allBundles.map { $0.path(forResource: filename, ofType: "json") }
        print("Scanned files are \(scan)")

        let scan2 = Bundle.allBundles.map { $0.url(forResource: filename, withExtension: "json") }
        print("Scan 2 are \(scan2)")

        guard let path = Bundle.main.path(forResource: filename, ofType: "json") else {
            print("Failed to find path for \(filename)")
            return nil
        }

        let data = try Data(contentsOf: URL(fileURLWithPath: path))
        print("Loaded data from path \(path)")
        let instance = try JSONDecoder().decode(Model.self, from: data)
        print("Decoded model instance of \(instance)")
        return instance
    }
